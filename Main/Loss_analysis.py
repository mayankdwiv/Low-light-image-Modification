...
for img in name_list:
    ...
    updated_ill_map = mod.update_illumination_map(
        illumination_map, weight_strategy)
    corrected_ill_map = gc.gamma_correct(np.abs(updated_ill_map), gamma)
    ...
    denoised_image = db3d.denoising_bm3d(
    ...
    print('LOE:', lc.loss_calculate(ref_il_map, den_il_map), '\n')
    ...
    cv2.imwrite(output_dir + img + '_modified.jpg', denoised_image_bgr)
